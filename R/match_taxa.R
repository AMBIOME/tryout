#' Taxon matching using Dyntaxa (https://dyntaxa.se) and WoRMS (http://www.marinespecies.org/)
#' 1.) Matches latin name and dyntaxa_id in the data with Taxon name and TaxonId in the supplied dyntaxa list (dyntaxa_list), 
#' 2.) Taxon matching using WoRMS.
#' @param names Vector of scientific names and IDs.
#' @param ask Ask user in case of multiple matches.
#' @return Data frame with scientific name, scientific name ID and match type.
#' @export
match_dyntaxa <- function(names, ask = TRUE) {
